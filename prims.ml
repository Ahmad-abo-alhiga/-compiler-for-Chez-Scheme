...
module Prims : PRIMS = struct
  ...
  (* This is the interface of the module. It constructs a large x86 64-bit string using the routines
     ...
     string of primitive procedures. *)
      let car =
    let asm_code= " mov r10, [rbp+4*8]" ^ "\n" ^
                  " CAR rax, r10" ^ "\n" in
     make_routine "car" asm_code ;; 
  
  let cdr=
    let asm_code= " mov r10, [rbp+4*8]" ^ "\n" ^
                  " CDR rax, r10" ^ "\n" in
    make_routine "cdr" asm_code ;;               
  
  let cons=
    let asm_code= "mov r10, qword[rbp + 4*8] " ^ "\n" ^
                  "mov r11, qword[rbp + 5*8]" ^ "\n" ^
                  "MAKE_PAIR(rax,r10,r11)" ^ "\n" in
     make_routine "cons" asm_code ;;
  ...
  let apply_ =
    let asm_code =
      ...
      make_routine "apply" asm_code   ;;

  let procs = String.concat "\n\n" [type_queries ; numeric_ops; misc_ops;car ;cdr;set_car;set_cdr;cons;apply_];;
